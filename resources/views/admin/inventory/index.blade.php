...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        async submitAddStock() {
            ...
            try {
                const response = await fetch('{{ route('admin.inventory.add-stock') }}', {
                    method: 'POST',
                    headers: {
        ...
        async submitDeleteStock() {
            ...
            try {
                const response = await fetch('{{ route('admin.inventory.delete-stock') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Accept': 'application/json',
        ...
        async initializeInventory() {
            if (!confirm('This will create inventory records for all existing products. Continue?')) return;

            try {
                const response = await fetch('{{ route('admin.inventory.initialize') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Accept': 'application/json',
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-white/50">Track and manage product stock levels.</p>
            </div>
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <a href="{{ route('admin.inventory.history') }}" class="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white/80 shadow-sm hover:bg-white/10">
                    View History
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div class="rounded-xl border border-white/10 bg-white/5 p-5 shadow-sm">
                ...
                <div class="mt-2 text-2xl font-semibold">{{ $totalItems }}</div>
            ...
            <div class="rounded-xl border border-amber-500/25 bg-amber-500/10 p-5 shadow-sm">
                ...
                <div class="mt-2 text-2xl font-semibold">{{ $lowStockCount }}</div>
            ...
            <div class="rounded-xl border border-rose-500/25 bg-rose-500/10 p-5 shadow-sm">
                ...
                <div class="mt-2 text-2xl font-semibold">{{ $outOfStockCount }}</div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <form method="GET" action="{{ route('admin.inventory.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        ...
                        @if($selectedCategory || $search)
                            <a href="{{ route('admin.inventory.index') }}" class="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white/80 hover:bg-white/10">
                                Clear
                            </a>
                        @endif
                    </form>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-white/60">
                                        ...
                                        <div>No inventory items found. Click "Initialize Inventory" to create records for all products.</div>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
