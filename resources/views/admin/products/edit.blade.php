...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="mt-6 space-y-5 rounded-[24px] border border-white/10 bg-white/5 p-6" x-data="{ category: @js(old('category', $product->category)), newCategory: @js(old('new_category', '')) }">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                ...
                <div class="mt-2 flex items-center gap-4">
                    ...
                    <div class="text-xs text-white/50">{{ $product->image ?? '—' }}</div>
                </div>
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <div class="mt-2 text-xs text-white/40">To add a new size option, add another row value and save.</div>
            </div>
