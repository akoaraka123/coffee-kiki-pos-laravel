                            ...
                            <div class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-sm">
                                <div class="mt-4 overflow-hidden rounded-xl border border-white/10 bg-black/30">
                                    <table class="min-w-full text-left text-sm">
                                        <tbody class="divide-y divide-white/10">
                                            <tr>
                                                <td class="px-4 py-3 text-white/70">Pay in Cash</td>
                                                <td class="px-4 py-3 text-right font-semibold text-emerald-200">₱<span x-text="formatPrice((dailyPayload.summary?.paid_sales?.cash || 0))"></span></td>
                                            </tr>
                                            <tr>
                                                <td class="px-4 py-3 text-white/70">Pay in G-Cash</td>
                                                <td class="px-4 py-3 text-right font-semibold text-sky-200">₱<span x-text="formatPrice((dailyPayload.summary?.paid_sales?.gcash || 0))"></span></td>
                                            </tr>
                                            <tr>
                                                <td class="px-4 py-3 font-semibold text-white">Total Sales</td>
                                                <td class="px-4 py-3 text-right text-lg font-bold text-white">₱<span x-text="formatPrice((dailyPayload.summary?.total_sales || 0))"></span></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <details class="mt-4 rounded-xl border border-white/10 bg-black/30 p-4">
                                    <summary class="cursor-pointer text-sm font-semibold text-white/80">Payment & Money Inventory Breakdown</summary>
                                    <div class="mt-3 grid grid-cols-1 gap-4 lg:grid-cols-2">
                                        <div class="rounded-xl border border-white/10 bg-white/5 p-4">
                                            <div class="text-xs font-semibold text-white/60">Money Inventory Breakdown</div>
                                            <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                                                ...
                                                <template x-if="(dailyPayload.money_inventory?.breakdown || []).filter(x => (x.quantity || 0) > 0).length === 0">
                                                    <div class="col-span-2 sm:col-span-4 rounded-xl border border-white/10 bg-[#111]/40 px-3 py-2 text-xs text-white/60">No money inventory saved.</div>
                                                </template>
                                            </div>
                                        </div>

                                        <div class="rounded-xl border border-white/10 bg-white/5 p-4">
                                            <div class="text-xs font-semibold text-white/60">Payment Entries</div>
                                            <div class="mt-3 space-y-2">
                                                <template x-if="(dailyPayload.payment_inventory?.entries || []).length === 0">
                                                    <div class="rounded-xl border border-white/10 bg-[#111]/40 px-3 py-2 text-xs text-white/60">No payment entries saved.</div>
                                                </template>
                                                <template x-for="e in (dailyPayload.payment_inventory?.entries || [])" :key="'pe-' + e.id">
                                                    <div class="rounded-xl border border-white/10 bg-[#111]/40 p-3">
                                                        <div class="flex items-center justify-between gap-3">
                                                            <div class="text-xs font-semibold text-white/80" x-text="(e.payment_type || '—').toUpperCase() + (e.created_at ? (' • ' + e.created_at) : '')"></div>
                                                            <div class="text-xs font-semibold text-white">₱<span x-text="formatPrice(e.received_amount || 0)"></span></div>
                                                        </div>
                                                        <div class="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                                                            <template x-for="it in (e.items || []).filter(x => (x.quantity || 0) > 0)" :key="'pei-' + e.id + '-' + it.denomination">
                                                                <div class="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[11px] text-white/80">
                                                                    <div class="font-semibold" x-text="'₱' + Number(it.denomination || 0).toLocaleString()"></div>
                                                                    <div class="text-white/60" x-text="'Qty: ' + Number(it.quantity || 0)"></div>
                                                                </div>
                                                            </template>
                                                        </div>
                                                    </div>
